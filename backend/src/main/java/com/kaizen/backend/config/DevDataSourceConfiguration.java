...
public class DevDataSourceConfiguration {
    ...
    public DataSource dataSource(Environment environment) {
        ...
        if (useEmbedded) {
            ...
            log.warn("Embedded Postgres container failed and H2 fallback is disabled; attempting configured JDBC data source.");
        }
    ...
    private DataSource tryBuildEmbedded(Environment environment) {
        try {
            ...
            embeddedContainer = new PostgreSQLContainer<>(image)
    ...
    private DataSource buildJdbcDataSource(Environment environment) {
        ...
        if (!StringUtils.hasText(url)) {
            ...
            String port = environment.getProperty("DB_PORT", "55432");
            String database = environment.getProperty("DB_NAME", "kaizen");
        ...
        return DataSourceBuilder.create()
            ...
            .url(url)
            .username(environment.getProperty("DB_USER", "kaizen"))
            .password(environment.getProperty("DB_PASSWORD", "kaizen"))
            .build();
    }

    private boolean getBoolean(Environment environment, String property, boolean defaultValue) {
        return Boolean.parseBoolean(environment.getProperty(property, String.valueOf(defaultValue)));
    }

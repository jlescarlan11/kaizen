...
public class OnboardingProgressController {
    ...
    @Operation(summary = "Delete onboarding progress", description = "Removes the durably persisted onboarding progress for the authenticated user.")
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "Progress deleted successfully."),
            @ApiResponse(responseCode = "401", description = "User must be authenticated.", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    private FundingSourceType resolveFundingSourceType(String fundingSourceType) {
        ...
        return FundingSourceType.fromValue(fundingSourceType)
                .orElseThrow(() -> new ValidationException(
                        java.util.List.of(new ValidationError("fundingSourceType",
                                "Funding source must be one of CASH_ON_HAND, BANK_ACCOUNT, or E_WALLET."))));
    }
}

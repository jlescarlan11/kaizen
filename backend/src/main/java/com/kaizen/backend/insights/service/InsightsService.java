...
public class InsightsService {
    ...
    public SpendingSummaryResponse getSpendingSummary(String email, LocalDateTime start, LocalDateTime end) {
        ...
        BigDecimal totalIncome = transactionRepository.sumAmountByTypeAndDateRange(account.getId(), TransactionType.INCOME, start, end);
        BigDecimal totalExpenses = transactionRepository.sumAmountByTypeAndDateRange(account.getId(), TransactionType.EXPENSE, start, end);
    ...
    public CategoryBreakdownResponse getCategoryBreakdown(String email, LocalDateTime start, LocalDateTime end) {
        ...
        List<CategoryBreakdownResponse.CategoryEntry> entries = results.stream()
            .map(r -> {
                ...
                return new CategoryBreakdownResponse.CategoryEntry(categoryId, categoryName, total, count, percentage);
            })
    ...
    public TrendSeriesResponse getSpendingTrends(String email, LocalDateTime start, LocalDateTime end, String granularity) {
        UserAccount account = getUserByEmail(email);
